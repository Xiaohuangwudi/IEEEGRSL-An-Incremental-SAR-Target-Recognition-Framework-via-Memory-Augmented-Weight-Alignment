...
from utils.data import iCIFAR10, iCIFAR100, iImageNet100, iImageNet1000, SAR
...
class DataManager(object):
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        idata = _get_idata(dataset_name)  #msatr
        idata.download_data()
        ...
        # Order
        order = [i for i in range(len(np.unique(self._train_targets)))]         # order: target classes
        if shuffle:
            np.random.seed(seed)
    ...
    def __getitem__(self, idx):
        ...
        if self.use_path:
            ...
            if img_array.dtype == np.float64 or img_array.dtype == np.float32:
                # Assuming the float values are in [0, 1] range
                if img_array.max() <= 1.0:
                    img_array = (img_array * 255).astype(np.uint8)
                else:
                    # Values are in [0, 255] range but stored as float
                    img_array = img_array.astype(np.uint8)
            
            # Handle different array shapes
            if len(img_array.shape) == 3 and img_array.shape[2] == 1:
                # Handle (H, W, 1) shape by squeezing or repeating the channel
                img_array = np.squeeze(img_array, axis=2)
                # If we need RGB, we can repeat the channel
                # img_array = np.repeat(img_array[:, :, np.newaxis], 3, axis=2)
            
            # Convert to PIL Image
            image = self.trsf(Image.fromarray(img_array))
